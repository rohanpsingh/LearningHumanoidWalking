...
class SteppingTask(BaseTask):
    ...
    def __init__(
        ...
        with open("utils/footstep_plans.txt") as fn:
    ...
    def calc_reward(self, prev_torque, prev_action, action):
        ...
        reward = dict(
            foot_frc_score=0.150
            * rewards.calc_foot_frc_clock_reward(
                self.l_foot_frc, self.r_foot_frc, self._phase, l_frc_fn, r_frc_fn, self._mass
            ),
            foot_vel_score=0.150
            * rewards.calc_foot_vel_clock_reward(self.l_foot_vel, self.r_foot_vel, self._phase, l_vel_fn, r_vel_fn),
    ...
    def reset(self, iter_count=0):
        ...
        for box, step in zip(boxes, sequence, strict=False):
