...
class RobotBase:
    def __init__(self, pdgains, dt, client, task, pdrand_k=0, sim_bemf=False, sim_motor_dyn=False):
        ...
        assert not (self.sim_bemf and self.sim_motor_dyn), (
    ...
    def _do_simulation(self, target, n_frames):
        ...
        assert target.shape == (self.client.nu(),), f"Target shape must be {(self.client.nu(),)}"
    ...
    def step(self, action, offset=None):
        ...
        if offset is not None:
            ...
            assert offset.shape == action.shape, f"Offset shape {offset} must match action shape {action.shape}"
