...
import numpy as np
import pytest
from conftest import DISCOVERED_ENVIRONMENTS, close_all_envs, get_all_env_instances
...
class TestMirrorSymmetry:
    ...
    def test_mirror_indices_valid_if_present(self, env_instance, env_name):
        ...
        for idx in mirrored_obs:
            ...
            assert abs_idx < obs_dim or abs(idx) < 1, f"Invalid mirror obs index: {idx}, obs_dim={obs_dim}"
        ...
        try:
            for env_name, env in envs:
                for attr in required_attrs:
                    assert hasattr(env, attr), f"Environment {env_name} missing attribute: {attr}"
        finally:
            close_all_envs(envs)
    ...
    def test_all_envs_return_consistent_step_signature(self):
        ...
        try:
            for env_name, env in envs:
                ...
                assert len(result) == 4, f"{env_name} step() should return 4 values, got {len(result)}"
                obs, reward, done, info = result
                assert isinstance(obs, np.ndarray), f"{env_name}: obs should be ndarray"
                assert isinstance(reward, (int, float)), f"{env_name}: reward should be numeric"
                assert isinstance(done, bool), f"{env_name}: done should be bool"
                assert isinstance(info, dict), f"{env_name}: info should be dict"
        finally:
            close_all_envs(envs)
    ...
    def test_all_envs_dimensions_consistent(self):
        ...
        try:
            for env_name, env in envs:
                ...
                obs = env.reset()
                assert obs.shape[0] == obs_dim, f"{env_name}: reset obs dim mismatch"
...
class TestDiscoveredEnvironments:
    ...
    def test_environments_discovered(self):
        ...
        assert len(DISCOVERED_ENVIRONMENTS) > 0, "No environments discovered under envs/"
