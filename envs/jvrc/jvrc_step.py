...
class JvrcStepEnv(JvrcWalkEnv):
    ...
    def draw_markers(self, marker_drawer):
        ...
        if hasattr(self.task, 'sequence'):
            for step in self.task.sequence:
                ...
                if step_pos not in [self.task.sequence[self.task.t1][0:3].tolist(),
                    ...
                    marker_drawer.add_marker(pos=step_pos, size=np.ones(3)*0.05,
                                             rgba=np.array([0, 1, 1, 1]), type=sphere)
                    marker_drawer.add_marker(pos=step_pos,
            ...
            marker_drawer.add_marker(pos=step_pos, mat=tf3.euler.euler2mat(0, np.pi/2, step_theta),
                                     size=arrow_size, rgba=np.array([1, 0, 0, 1]), type=arrow)
            marker_drawer.add_marker(pos=step_pos, size=np.ones(3)*target_radius,
                                     rgba=np.array([1, 0, 0, 0.1]), type=sphere)

            # t2 target (blue)
            step_pos = self.task.sequence[self.task.t2][0:3].tolist()
            step_theta = self.task.sequence[self.task.t2][3]
            marker_drawer.add_marker(pos=step_pos, size=np.ones(3)*0.05,
                                     rgba=np.array([0, 0, 1, 1]), type=sphere)
            marker_drawer.add_marker(pos=step_pos, mat=tf3.euler.euler2mat(0, np.pi/2, step_theta),
                                     size=arrow_size, rgba=np.array([0, 0, 1, 1]), type=arrow)
            marker_drawer.add_marker(pos=step_pos, size=np.ones(3)*target_radius,
                                     rgba=np.array([0, 0, 1, 0.1]), type=sphere)
        ...
        marker_drawer.add_marker(pos=[goalx[0], goaly[0], 0], size=np.ones(3)*0.05,
                                 rgba=np.array([0, 1, 1, 1]), type=sphere)
        marker_drawer.add_marker(pos=[goalx[0], goaly[0], 0],
                                 mat=tf3.euler.euler2mat(0, np.pi/2, goaltheta[0]),
                                 size=arrow_size, rgba=np.array([0, 1, 1, 1]), type=arrow)
        marker_drawer.add_marker(pos=[goalx[1], goaly[1], 0], size=np.ones(3)*0.05,
                                 rgba=np.array([0, 1, 1, 1]), type=sphere)
        marker_drawer.add_marker(pos=[goalx[1], goaly[1], 0],
                                 mat=tf3.euler.euler2mat(0, np.pi/2, goaltheta[1]),
                                 size=arrow_size, rgba=np.array([0, 1, 1, 1]), type=arrow)

        # Draw feet poses (gray)
        lfoot_orient = (tf3.quaternions.quat2mat(self.task.l_foot_quat)).dot(
            tf3.euler.euler2mat(0, np.pi/2, 0))
        rfoot_orient = (tf3.quaternions.quat2mat(self.task.r_foot_quat)).dot(
            tf3.euler.euler2mat(0, np.pi/2, 0))
        marker_drawer.add_marker(pos=self.task.l_foot_pos, size=np.ones(3)*0.05,
                                 rgba=[0.5, 0.5, 0.5, 1], type=sphere)
        marker_drawer.add_marker(pos=self.task.l_foot_pos, mat=lfoot_orient,
                                 size=arrow_size, rgba=[0.5, 0.5, 0.5, 1], type=arrow)
        marker_drawer.add_marker(pos=self.task.r_foot_pos, size=np.ones(3)*0.05,
                                 rgba=[0.5, 0.5, 0.5, 1], type=sphere)
        marker_drawer.add_marker(pos=self.task.r_foot_pos, mat=rfoot_orient,
                                 size=arrow_size, rgba=[0.5, 0.5, 0.5, 1], type=arrow)

        # Draw origin axes
        marker_drawer.add_marker(pos=[0, 0, 0], size=np.ones(3)*0.05,
                                 rgba=np.array([1, 1, 1, 1]), type=sphere)
        marker_drawer.add_marker(pos=[0, 0, 0], mat=tf3.euler.euler2mat(0, 0, 0),
                                 size=[0.01, 0.01, 2], rgba=np.array([0, 0, 1, 0.2]), type=arrow)
        marker_drawer.add_marker(pos=[0, 0, 0], mat=tf3.euler.euler2mat(0, np.pi/2, 0),
                                 size=[0.01, 0.01, 2], rgba=np.array([1, 0, 0, 0.2]), type=arrow)
        marker_drawer.add_marker(pos=[0, 0, 0], mat=tf3.euler.euler2mat(-np.pi/2, np.pi/2, 0),
                                 size=[0.01, 0.01, 2], rgba=np.array([0, 1, 0, 0.2]), type=arrow)
